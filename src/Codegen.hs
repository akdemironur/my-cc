...
import Tacky
import TypeCheck
import Util (is32BitRange, roundAwayFromZero)
...
formatBinaryInstruction op t src dst = case (op, src) of
  (AsmSal, Register reg) -> formatInstruction (codeEmission AsmSal) Nothing [codeEmissionReg1 reg, oprEmission t dst]
  (AsmSar, Register reg) -> formatInstruction (codeEmission AsmSar) Nothing [codeEmissionReg1 reg, oprEmission t dst]
  (AsmSal, _) -> formatInstruction (codeEmission AsmSal) Nothing [oprEmission t src, oprEmission t dst]
  (AsmSar, _) -> formatInstruction (codeEmission AsmSar) Nothing [oprEmission t src, oprEmission t dst]
